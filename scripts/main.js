...
function upscaleImages() {
    ...
    var idealwidth = caw;
     ...
     for(i=0;i<img_cimg_scales.length;i++) "<="" *="" +="" b="" build="" classname="img_cimg_scale_classes[i];" classscale="img_cimg_scales[i];" classscale);="" debug="" dinfo="" idealwidth="Math.round(caw" idx="" if($(this).hasclass(classname))="" info:="" shouldupdate="1;" some="" {=""> (class " + classname + ")<br/>";
        ...
        if (shouldupdate &gt; 0) {
          ...
          for(i=0;i<img_avail_widths.length;i++) <="img_avail_widths[i])" chosen_width="img_avail_widths[i];" for(i="img_avail_widths.length-1;i" if(idealwidth="" {="" }="">=0;i--) {
...
</img_avail_widths.length;i++)></img_cimg_scales.length;i++)>